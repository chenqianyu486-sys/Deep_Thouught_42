...
async def run_evaluate_phase(state: OptimizerState, deps: NodeDeps) -> LoopPhase:
    ...
    while True:
        ...
        if step_state:
            ...
            if deps.compat is not None:
                deps.compat.add_message("user",
                    f"[NOTE] report_step_state required. Current best WNS: {state.timing.best_wns}ns. Decide: NEXT_ITERATION, SWITCH_STRATEGY, DONE, or CONTINUE. If strategy improved WNS, prefer CONTINUE.")
