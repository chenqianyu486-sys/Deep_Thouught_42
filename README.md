...
| 17o | 高扇出/热点网络名一致性（P0-1，2026-07-12） | run-20260712_013828 回归根因：LLM 在 ANALYZE 拿到真实高扇出网但未持久化入 state，rollback 后 EXECUTE 缓存空，LLM 幻觉网络名（用时序报告热点标签 `M1[21]` 而非真实 `M1w[21]`）喂给 fanout 工具致 -1.220ns 回归。修复五点：(1) `phase_analyze.py` 新增 handler 将 `vivado_get_critical_high_fanout_nets` live 结果解析入 `state.timing.high_fanout_nets` + 第 4 个 ANALYZE 入口自动刷新块（stale 时重新获取，rollback 后恢复）；(2) `VivadoMCP` `extract_critical_path_cells` 对 `top_delay_nodes` net 节点用 `get_property PARENT` 解析为父网络名（消除热点标签与网表名歧义）；(3) `phase_execute.py` Fanout 注入改为 overriding（`high_fanout_nets` 非空时覆盖 LLM nets，附 `[DATA INTEGRITY]`，匹配 critical_paths 模式）；(4) `optimize_fanout_batch` 加 `MIN_FANOUT_TO_SPLIT=50` 守卫跳过低扇出网（此前 fanout=2 也被拆分），skipped 不递增 `successful_count` 触发 `should_skip_chain_for_empty_result` 跳过 P&R chain；(5) 引导层补齐（`prepare_context.py`+`state_space.py`）：`BASE_FORMAT_GUARD` 新增 NET NAME CONTRACT 段（fanout `nets` 属 NET 名空间≠cell 名，权威来源 Module 4 `high_fanout_nets`，`delay_hotspots` `[net]` 标签=net 名/cell 类型标签=cell 名，声明 auto-inject+override，禁止手抄时序报告 net 名 `M1[21]`≠`M1w[21]`）；Fanout 补进 STALE DATA HANDLING/EXECUTE addendum 自动注入说明（对齐 `NETLIST_MODIFYING_STRATEGIES`）；Dashboard 追加 inline 命名空间注释 |
| 17p | 其余工具上下文工程审计（2026-07-12） | 4-agent 并行排查 + 7 项亲验，11 处修复/1 处有意跳过：F6 `design_data_read`/`design_data_list_snapshots` 未注册 schema（`dcp_optimizer.py` 补两工具）；F3 CellReplication auto-inject 补进 FORMAT_GUARD 三列表；F5 "ExploreArea"->"ExploreWithAreaDuplication" 对齐 Vivado 2025.1 白名单（6 处：3 skill + server.py×2 + rapidwright_tools.py）**[2026-07-12 P0 修正：方向搞反，已回退为 ExploreArea]**；F7 删 SYSTEM_PROMPT CONTINUE 的 `(ANALYZE or EXECUTE only)`（与 EVALUATE 矛盾）；F2/F4 新增 PIN NAME CONTRACT（lut_input_cone 需 PIN 名=cell+/I0；PinSwap 不自动注入、对象格式 `[{cells:[...]}]`）；F8 指令白名单拒绝改 JSON（5 处，原纯文本不被 `_MCP_ERROR_PATTERNS` 检测）；F1 `critical_paths_data` 补进 CELL_PARAM_SPECS；F10 [CELL REGISTRY] NOTE 补 module 名警告；F11 cached_high_fanout_nets 描述补空/陈旧；F12 SYSTEM_PROMPT 策略列表补 PlaceRouteDirectiveExplore+CongestionRouteExplore；F9 physopt enum 有意跳过（保留 retiming BLOCKED 警告）。详见 architecture.md |
| 18a | Vivado 2025.1 man page 工具链正确性审计（P0，2026-07-12） | 对照 `/home/qianyu/2025.1/Vivado/doc/eng/man/` 官方 man page 审查全部 Vivado TCL 命令用法。3 项 P0 修复：(C1) `unplace_cells` TCL 命令改为单数 `unplace_cell`（man page 确认 Vivado 命令为单数，复数形式被 Vivado 拒绝）；(C2) `run_tcl` 工具描述中移除不存在的 `report_critical_paths` 命令推荐，替换为 `report_timing -max_paths N -return_string`；(C3/C4) `OPT_SAFE_DIRECTIVES` 修正：移除 4 个无效 directive（`ExploreWithAreaDuplication`/`NoBramOptimization`/`NoDspOptimization`/`DataSpreadMem`，均不在 man page 或二进制库中），添加 3 个有效 directive（`ExploreArea`/`ExploreWithRemap`/`RQS`），同步更新 opt_design enum、3 个 skill ALLOWED_DIRECTIVES、2 个 RapidWrightMCP 描述、rapidwright_tools.py docstring、strategy_library.py 名称。涉及 8 文件 15 处编辑，470 项测试全通过 |
| 18b | Vivado 2025.1 man page 工具链正确性审计（P1-P3，2026-07-12） | P1（已在 P0 完成）：`report_critical_paths` 引用移除。P2：directive 白名单补全 — `PLACE_SAFE_DIRECTIVES` 从 12→23（补全 man page 全部 22 个有效 directive + Default，新增 `WLDrivenBlockPlacement`/`ExtraNetDelay_high/low`/`SSI_SpreadSLLs`/`SSI_BalanceSLLs`/`SSI_BalanceSLRs`/`SSI_HighUtilSLRs`/`RQS`/`Auto_1/2/3`）；`ROUTE_SAFE_DIRECTIVES` 从 7→10（补全 `MoreGlobalIterations`/`AdvancedSkewModeling`/`AlternateCLBRouting`）；`PHYSOPT_SAFE_DIRECTIVES` 的 `Default` 保留并加注释（man page 非穷尽列出，二进制确认有效）；opt_design handler 添加 `_is_unrecognized_directive_error` 回退（与 place/route handler 对齐，防止白名单漂移时静默回退）。P3：`Vivado_RapidWright_Usage_Reference.md` 4 个命令的 directive 表 + 语法块全面修正（opt_design 8 directive、place_design 22 directive、route_design 10 directive、phys_opt_design 11 directive 含 BLOCKED 标记；移除全部策略预设名误列为 directive 的条目；修正 `-rewire`→`-memory_rewire_opt` 等语法错误）。572 项测试全通过 |
| 18c | `Vivado_RapidWright_Usage_Reference.md` 全面校验与纠正（2026-07-12） | 基于 `vivado <cmd> -help`/man page/`set_property` 实测 + RapidWright `javap`/`RELEASE_NOTES.TXT` 对参考文档做 18a/18b 未覆盖的纠正。**Vivado 侧**：(1) `synth_design` 语法+directive 表重写（移除 8 个不存在 directive 如 `FlowOptimized_high`/`PerformanceRetiming`/`AggressiveExplore` 与 6 个不存在选项如 `-retiming`/`-max_buram`/`-hierarchical_block*`/`-fanout_limit`/`-mvcstyle`/`-sweep`，补全 12 个合法 directive，经 `set_property STEPS.SYNTH_DESIGN.ARGS.DIRECTIVE` 实测含 man page 未列的 `PowerOptimized_high/medium`）；(2) `report_timing_summary`/`report_timing`/`report_design_analysis` 语法重写（移除 `-min_slack`/`-max_slack`/`-include_headers`/`-rise`/`-fall`/`-logic_level_depth` 等不存在选项）；(3) `open_checkpoint -strict`->`-ignore_timing`，`write_checkpoint` 补 `-cell`/`-encrypt` 等，`route_design -unroute`/`-pins` 修正，`place_design` 补 `-subdirective`/`-eco` 等，`phys_opt_design` 补 `-cell_group_opt`/`-equ_drivers_opt`；(4) §7 实现策略全表重写：78 个候选名经 `set_property strategy` 在 xcvu3p+xcvu9p 双器件实测仅 17 个有效（默认策略为 `Vivado Implementation Defaults` 非 `Default`），64 个报 "not supported by flow 'Vivado Implementation 2025'"，新表给出 17 策略的权威 opt/place/phys_opt/route 内部指令映射；(5) §1.4 启动器参数移除不存在的 `-notrace`/`-lic_waittime`；(6) §5.3 仅 `report_design_analysis` 有 `-json`；(7) §4.5 `add_aps_to_pblock`->`delete_pblock`。**RapidWright 侧**（bundled 2025.2.1-beta）：§8.2 版本表 10 个发布日期系统性偏早约 1 年，全纠正（如 2025.2.1-beta 2025-02-19->2026-02-18）；"固定 commit f63afef" 标注为不可核对（RapidWright 以普通目录 vendored 非 gitlink，实际 bundled 为 2025.2.1-beta）；§9.1 包名纠正（`ecopt`->`eco`、`route`->`rwroute`、删 `place` 包）；§9.2 各类方法表大面积纠正（Net `getSinks`->`getSinkPins`、`getFanout`->`getFanOut`、`isPIPPlaced`->`hasPIPs`、`setSource(PortInst)`->`setSource(SitePinInst)`、`addSink`->`addPin`、`disconnect`->`unroute`；Device 删 `getSites`/`getAllPIPs`/`getWires`/`getRootTimingModel`；Cell `getType`/`getPins`/`unplace` 不存在；Site `getType`->`getSiteTypeEnum`；Tile `getCol`->`getColumn`；PIP `isPIPDownhill`->`getPIPType`；BEL `getType`->`getBELType`；SiteInst `place(Cell,BEL)`->`place(Site)`；SitePinInst `getCell`/`isOutward` 不存在）；§9.3 枚举纠正（`AWS_F2` 不存在、`NetType` 值 `WIRE/GND/VCC/UNKNOWN` 非 `SIGNAL`、`BELType`->`BELClass`、`PIPDirection`->`PIPType`）；§11.1 RWRoute CLI 实为位置参数 `<input> <output>` 非 `--flag`，包名 `rwroute`；§11.2-11.4 工具类方法表纠正（`addCell`->`createCell`、`replaceCell`->`refactorCell`、`mergeNets`/`splitNet` 不存在；DesignTools `optimizeFanout`/`analyzeFabricForPblock`/`reportTimingSummary`/`parallelRouterInit` 不存在；VivadoTools `reportPlaceStatus`/`getVivadoDcpLoadError`/`checkDcpAgainstVivado`/`runVivadoWithArguments` 不存在，应为 `placeAndRouteDesignAndGetStatus`/`roundTripDCPThruVivado`/`runTcl`）；§8.5 依赖版本精确化。所有命令语法/指令/策略经本地 Vivado 2025.1 实测，所有 API 方法经 `javap` 核对 |
...
| 24c | num_paths 链参数 + 自由编排 + 统一 block（P2，2026-07-13） | ①B：opt_design 族链 `vivado_extract_critical_path_cells` 的 `num_paths` 由 LLM 经 skill 参数控制（`extract_num_paths` 经 `args_from_skill` 注入，默认 10）。①C：`strategy_name` enum 新增 `CUSTOM`--无主工具，EXECUTE 不收窄工具集，LLM 自由编排 Vivado/RapidWright 工具（无 auto-chain，等价性靠工具级守卫 + DCP 验证）。②D：统一 block 语义--`[BLOCKED]` 仅真阻断（regression + 结构性 strategy_ineffective，regression 新加入显示）；`no_improvement`/`strategy_not_applicable` 改为 `[PRIOR FAIL]` 软标记（可选，带上下文），消除「显示 BLOCKED 但可选」矛盾。catalog 四态：`[RETRY]`/`[BLOCKED]`/`[COMBO COOLED]`/`[PRIOR FAIL]` |
