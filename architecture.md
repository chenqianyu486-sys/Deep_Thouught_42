...
- **无策略引导**（2026-06）：`design_delay_profile` 不再附带 `strategy_hint`，`_append_architecture_hints()` 重命名为 `_append_architecture_insights()`，仅输出纯数据描述。**2026-07-14 进一步去除策略强制/暗示**：① 移除迭代1强制 PBLOCK 的硬覆盖（`phase_select_strategy` 不再预选策略，LLM 依据 Dashboard 客观数据自选）；② CUSTOM 升为一等对等策略（SELECT/EXECUTE FORMAT_GUARD + `_append_skill_guidance` 新增 CUSTOM 分支，目录策略无效/不适配时 LLM 可手动编排任意 EXECUTE 工具，无 auto-chain）；③ SYSTEM_PROMPT 剥离 PBLOCK/PhysOpt/Fanout 主观成功率先验，仅保留 rapidwright/Vivado 耗时与成本等客观工具事实；④ `_append_skill_guidance` 的 "avoid: vivado_run_tcl - use the tool above" 改为中性信息（tcl 偏好仍由 SYSTEM_PROMPT automation 规则客观覆盖）。**保留** `_auto_deepen_hint`（鼓励 LLM 深耕有效策略）与 `next_strategy_hint`（LLM 自身跨阶段意图，非框架施加的暗示）。
...
- **`strategy_catalog` 排除机制（2026-07 更新）**: `strategy_ineffective`（TTL=1）、`no_improvement`（TTL=3）、`strategy_not_applicable`（TTL=5）、`regression`（TTL=2）在 catalog 中标为 `[BLOCKED]` 占位符（含剩余轮数/原因）；`tool_error`/`data_quality_error` **不再移出 catalog**（P0 ②C，2026-07-13）：带 `detail` + 剩余重试次数标为 `[RETRY: ...]` 显示在 available 列表（`get_strategy_catalog(retryable_strategies=...)`），LLM 可调参重选；连续 `RETRY_BUDGET=2` 次后升级 `strategy_ineffective`（见下）。排除逻辑在 `inject_merged_dashboard()` 中拆分 retryable vs blocked 两组。`regression` 经 `_get_permanently_blocked_strategies` 跨迭代硬阻断（run-20260711_164134：回归策略此前仅本迭代冷却，下迭代被重选）。
...
- **重试预算 + 升级（P0 ②B，2026-07-13）**: `FailedStrategyRecord.retry_count` 字段累计可重试失败（`tool_error`/`data_quality_error`/`unknown`）的重试次数；`record_strategy_failure` 在更新同策略的可重试记录时 `retry_count += 1`，达 `RETRY_BUDGET=2` 后**升级**为 `strategy_ineffective`（TTL=1）。升级方向是 TTL 0->1（更严格），与「失败分类不覆盖」的 no-downgrade 守卫一致（仅当 existing 本身为可重试类时触发，已冷却的策略不受影响）。这把「失败->冷却换策略」改为「失败->看根因->调参重试 N 次->仍失败才冷却」。
- **结构化错误信封（P0 ③A/③B，2026-07-13）**: `optimizer/pure/tool_error_classify.py:classify_tool_error(tool_name, error)` 将工具错误分类为 `category`（bad_cell_name/bad_directive/tcl_blocked/timeout/vivado_error/rw_error/schema_validation/partial_failure/rate_limited/unknown）+ `fix_hint`（可执行修正建议）+ `retryable`。`tool_summary.summarize_tool_result()` 在 `status=error/failed` 时于摘要注入 `error_category`/`fix_hint`/`retryable` 三段（bypass 与 general 两条返回路径均覆盖）。EVALUATE 阶段工具错误额外注入 `[EVAL ERROR] {tool} failed (category=...) {fix_hint} retryable=...` user message（`phase_evaluate.py`，此前仅 log）。
- **参数覆盖显式 opt-out（P0 ①A，2026-07-13）**: pblock/fanout skill `inputSchema` 新增 `trust_llm_input: bool`（`RapidWrightMCP/server.py`）。`phase_execute.py` 在 MCP 调用前 `tool_args.pop("trust_llm_input")` 消费该 flag：为 True 时 pblock 用 LLM 的 `critical_path_cells`（仍经 `EntityRegistry.contains` 校验，全部合法才采纳，否则回退 state 数据 + 警告）、fanout 用 LLM 的 `nets`（无 registry，保留 tool 端 `MIN_FANOUT_TO_SPLIT` 守卫）；`resource_multiplier` 2.0x 地板不放开。默认 False（沿用 verified state 数据覆盖，防 -1.220ns 幻觉网络回归）。
- **按参数组合冷却（P1 ②A，2026-07-13）**: `FailedStrategyRecord.param_signature`（directive 对等组合指纹，`execute_contracts.compute_param_signature(strategy, tool_args)`）+ `StrategyState.current_param_signature`（EXECUTE 入口写入，SELECT 重置）。`record_strategy_failure` 按 `(strategy, param_signature)` 去重--同一策略不同 directive 组合是独立记录，重试预算（P0 `retry_count`）按组合独立。升级后的组合（tool_error->strategy_ineffective，`param_signature!=""`）**不在 SELECT_STRATEGY 阻断整策略**：`_get_permanently_blocked_strategies` 仅阻断 `param_signature==""` 的策略级失败（regression/结构性 strategy_ineffective/strategy_not_applicable/no_improvement）；升级组合改由 EXECUTE `combo_is_cooled` 守卫拦截（命中则跳过 MCP 调用 + emit `[COMBO COOLED] ... 选同一策略换不同 directive 组合`）。catalog 三态：`_retryable`（tool_error 未升级，available 行 `[RETRY: <detail> [combo] - N left]`）/ `_blocked`（param_signature=="" TTL 活跃，`[BLOCKED]`）/ `_combo_cooled`（param_signature!="" strategy_ineffective TTL 活跃，available 行 `[COMBO COOLED: <combo> - unblocks in N iter]`，优先级高于 `[RETRY]`）。data-only 策略（Fanout/NetSwap）`param_signature=""` -> 策略级（行为同今日）。`state_space.strategy_outcomes` 失败表新增 `param_signature` 字段。
- **跨阶段失败记忆（P1 ③C，2026-07-13）**: `PhaseHandoff.recent_failures: list[str]`，由 `build_phase_handoff` 从 `state.iteration.tool_errors[-3:]` 构造（`"{tool}: {result[:120]}"`），在 `to_phase_context_string` 渲染 "Recent Failures:" 段。`tool_errors` 仅在 `iteration_start` 清空，故补的是迭代内跨阶段（EXECUTE->EVALUATE->SELECT）的即时记忆；跨迭代靠 `failed_strategies`（P0 ②C 已上 detail）。三处 `build_phase_handoff` 调用点（execute/select/analyze）均传入。
- **num_paths 链参数（P2 ①B，2026-07-13）**: opt_design 族（opt_design/combinational_rebalancing/lut_muxf_repack）skill wrapper 新增 `num_paths: int = 10` 参数，附到 skill result 的 `extract_num_paths` 字段；链步 `vivado_extract_critical_path_cells` 加 `args_from_skill: {"num_paths": "extract_num_paths"}`，`resolve_chain_step_arguments` 将 skill 的 `extract_num_paths` 覆盖硬编码 10。LLM 可控制链后提取的关键路径数。两份链（constants.py/tool_chain_policy.py）同步（muxf_tree/flatten_lut_cascade 链亦有 args_from_skill 但其 wrapper 暂未附 `extract_num_paths`，dormant 回退 10）。
- **自由编排模式 CUSTOM（P2 ①C，2026-07-13）**: `strategy_name` enum 新增 `CUSTOM`，`STRATEGIES` 加 CUSTOM 条目（trigger: 无预定链适合时直接编排）。CUSTOM 不在 `STRATEGY_MAP` -> `get_strategy_primary_tool("CUSTOM")` 返回 None -> `filter_tools_for_phase` 不收窄，保留完整 `PHASE_TOOLS[EXECUTE]`（所有策略主工具 + Vivado place/route/opt/phys_opt + 分析工具）。LLM 在 EXECUTE 直接调用工具自由编排，无 auto-chain、无主工具 override；`EXEC_DONE` 退出。等价性靠工具级守卫（retiming 黑名单/directive 白名单）+ DCP 验证兜底。机制等同既有「未知策略保留全工具集」路径（`test_unknown_execute_strategy_keeps_phase_toolset`），正式化为 enum 值。
- **统一 block 语义（P2 ②D，2026-07-13）**: 消除「软阻止」矛盾。catalog 四态：`_blocked`（真阻断，TTL 内不可选）= regression + 结构性 strategy_ineffective（param_signature==""），**regression 新加入显示**（与其 `_get_permanently_blocked_strategies` 硬阻断一致）；`_soft_failed`（新）= no_improvement + strategy_not_applicable（param_signature=""），available 行 `[PRIOR FAIL: <reason> - selectable, unblocks in N iter]`，**不硬阻断**（SELECT 守卫不变，仍可选）；`_combo_cooled`（param_signature!="" strategy_ineffective）+ `_retryable`（tool_error 未升级）同 P1。`get_strategy_catalog` 优先级 combo_cooled > soft_failed > retryable。`_get_permanently_blocked_strategies` 不变（strategy_ineffective + regression，param_signature==""）。SELECT 守卫的 `[BLOCKED]` 消息由纯函数 `blocking_reason_for_strategy` 生成（区分 regression「made WNS worse」vs strategy_ineffective「temporarily ineffective」+ 正确解封轮数；此前 regression 被误报为「unblocks in 0 iterations」）。`strategy_outcomes` 失败表的 `blocked_info` 同步四态：soft 失败标 `(soft, unblocks in N iter)`、regression 标 `(blocked - regression, ...)`。
- **EXHAUSTED 文案收紧（run-20260713_050453 复盘）**: 该运行 LLM 在 EXECUTE 误用 `EXHAUSTED`（语义「CellReplication 这一个策略不适配」），但 EXHAUSTED 是终止性的（`phase_execute:is_done=True`），导致运行结束、未试到 LLM 自己推荐的 PlaceRouteDirectiveExplore。FORMAT_GUARD 收紧：EXECUTE 阶段新增 `EXEC_DONE` vs `EXHAUSTED` 区分指引（EXEC_DONE=本策略结束去 EVALUATE 换策略；EXHAUSTED=终止整个运行，仅当所有可达策略已试/已 block 且 2+ 连续无改进时用，误用会放弃未试策略）；EVALUATE 的 EXHAUSTED 行从「end this iteration」改为「TERMINAL，ends the ENTIRE run」+ 要求 2+ 连续无改进。
- **next_strategy_hint 跨阶段策略意图（P2 route-2，2026-07-13）**: `report_step_state` 新增可选 `next_strategy_hint` 字段;LLM 在 EXEC_DONE/SWITCH_STRATEGY 时可填「下一个策略名」。`StepState.next_strategy_hint` 经 phase_execute/phase_evaluate 捕获到 `StrategyState.pending_next_strategy_hint`;SELECT_STRATEGY 入口由纯函数 `build_strategy_hint_message(hint, STRATEGIES)` 校验(无效/空静默丢弃)后注入 `[STRATEGY HINT] ... (suggestion, NOT enforced)` user message,消费即清空。给 LLM 一个合法的「换到 X」出口,避免因无法在一次信号表达「done + 换 X」而误用 EXHAUSTED。flow_control 仍是单字符串 enum(不支持复合控制字),此为补充字段非复合信号。
- **flow_control 工具描述注入语义（P2 提示词工程，2026-07-13）**: 复盘 run-20260713_050453 发现 `filter_tools_for_phase` 按 phase 裁剪 flow_control enum 时,把 description **覆盖为裸列表**（"Valid signals for EXECUTE: EXEC_DONE, CONTINUE, EXHAUSTED"）--LLM 在工具描述里看不到各信号语义,只能靠 FORMAT_GUARD,这是 EXHAUSTED 误用的诱因之一。改为注入每信号一行语义（`_FLOW_CONTROL_BLURBS`: EXEC_DONE「NORMAL way to move on, even if bad fit」、EXHAUSTED「TERMINAL, NOT for bad-fit」等）。同步修正 schema 基础描述的歧义:NEXT_ITERATION（原「significant improvement, diminishing returns」自相矛盾->「end iteration, start fresh」）、SWITCH_STRATEGY（原「failed or try another」冗余->「abandon current, pick different」）、EXHAUSTED（原「all tried, no improvement」-> TERMINAL + 2+ 连续无改进 + 非不适配）。
...
```
> **数据完整性保护（2026-06）**: LLM 可能通过 TCL 提取含扇出分支的污染数据。上述注入已改为**始终覆盖** LLM 提供的参数，并记录 warning 日志 + 向 LLM 上下文注入 `[DATA INTEGRITY]` 警告。同一机制也应用于 `critical_paths` 参数（CombinationalRebalance / LUTMUXFRepack / MUXFTreeReorder / LUTCascade 策略）。**CellReplication auto-inject（2026-07-12，`dcp_optimizer_run-20260711_230953`）**：`rapidwright_replicate_critical_cells` 的 schema 要求 `critical_paths` 为对象数组 `[{"cells":[{"name","delay","type","fanout"}]}]`，与上述 string-array sibling 工具形状不同，且原先不在 auto-inject 列表，导致 LLM 三次 input validation 失败。现为其新增专属 override 分支，用 `derive_cells_rich()`（`optimizer/pure/critical_path.py`，从 `CriticalPathEntry.nodes` 构建富对象）注入经核验的 state 数据；skill 端 `_identify_high_delay_cells` 再用 design 查真实 output-net fanout 覆盖传入值。**2026-06 增强**：注入逻辑统一改用 `extract_registry_cells_for_inject()`（注册表 + critical paths），替换原 pblock 子串过滤与零过滤的不一致实现；`rapidwright_optimize_cell_placement` 新增同源 auto-inject（详见 §12.4）。
...
**新鲜度写入路径统一（R1，2026-07-12）**：`field_freshness: dict[str, str]` 与 `critical_paths_stale: bool` + `critical_paths_stale_reason: str` 此前是同一事实的两个并行表征，由 9 个调用点各自手工同步，反复漂移（F4/F5 "stale=false [stale]" 矛盾、chain-step 仅写 bool 漏写 field）。现统一为 `optimizer/pure/freshness.py` 三个纯函数：`mark_all_fields_stale(timing, reason=)` 一次性置全部字段 stale + bool=True + reason；`mark_critical_paths_stale(timing, reason=)` 仅置 critical_path_cells 字段 + bool + reason（修复 chain-step 漏写 field 的潜在漂移）；`mark_critical_paths_fresh(timing)` 清 bool + reason + 置 field=fresh。所有 9 个生产调用点（phase_execute×5、phase_select、phase_evaluate、rollback、critical_path）改用这三个函数，`rg "critical_paths_stale\s*=\s*(True|False)"` 在生产代码中零命中（仅 freshness.py/state.py/tests 有）。未采用 per-field `DataCurrency` dataclass 替换 `dict[str,str]`——8/9 字段无需 reason，全量替换触及 ~30 站+序列化器+测试，违反 Simplicity First。

**数据驱动阶段入口刷新（R2，2026-07-12）**：ANALYZE 入口此前有 4 个硬编码刷新块（timing_summary / design_info / resource_utilization / high_fanout_nets），SELECT 入口有 1 个，每块 ~25 行且各自 try/except。现统一为 `freshness.py` 中 `RefreshSpec` 声明式表 + `run_phase_entry_refresh(state, deps, phase)` 异步函数。`ANALYZE_REFRESH_SPECS` 含 6 个 spec：原 4 字段 + **route_status**（新增，此前 LLM 须手动刷新）+ **congestion_data**（新增，~2.5s RapidWright，一次/阶段入口）。`SELECT_REFRESH_SPECS` 仅 timing_summary（带 routed-design 守卫：非 routed 不采纳 WNS）。`critical_path_cells` 故意排除——重提取昂贵且依赖目标选择，由 LLM 自主决定。每个 spec 含 `field_name`/`tool`/`args`/`post_process` 回调/可选 `condition` 门控；新增字段现在只需加一个 spec 条目而非 ~25 行新块。
...
- **F5 "ExploreArea" 指令名跨层不一致**（`skills/{opt_design,lut_muxf_repack,combinational_rebalancing}_strategy.py` + `RapidWrightMCP/{server,rapidwright_tools}.py` 共 6 处）：skill 的 ALLOWED_DIRECTIVES + 描述用 "ExploreArea"，但 Vivado 2025.1 `OPT_SAFE_DIRECTIVES`/opt enum 用 "ExploreWithAreaDuplication"，skill 放行后透传 Vivado 被拒致整链失败。6 处统一改为 "ExploreWithAreaDuplication"。**[2026-07-12 P0 修正]** 经对照 `/home/qianyu/2025.1/Vivado/doc/eng/man/opt_design` man page 审计，F5 修复方向搞反：man page 列出的有效 directive 是 `ExploreArea`（非 `ExploreWithAreaDuplication`），后者在 man page 和 Vivado 二进制库中均不存在。同时发现 `OPT_SAFE_DIRECTIVES` 中 `NoBramOptimization`/`NoDspOptimization`/`DataSpreadMem` 也不在 man page 中。已将 `OPT_SAFE_DIRECTIVES` 修正为 man page 验证的 8 个有效 directive（`Default`/`Explore`/`ExploreArea`/`ExploreWithRemap`/`ExploreSequentialArea`/`RuntimeOptimized`/`RQS`/`AddRemap`），6 处 skill/MCP 引用同步回退为 `ExploreArea`。另修复 `unplace_cells` TCL 命令（应为单数 `unplace_cell`）和 `report_critical_paths` 不存在命令引用。
...
# P0 ②B（2026-07-13）: 可重试失败（tool_error/data_quality_error/unknown）累计
# retry_count，达 RETRY_BUDGET=2 升级 strategy_ineffective（current+1）
RETRY_BUDGET = 2
...
| `rapidwright_replicate_critical_cells` | ✅ **新增** | `derive_cells_rich()`（从 `CriticalPathEntry.nodes` 构建富对象 `{name,delay,type,fanout}`；skill 端再用 design 查真实 fanout） |
...
**验证**：`optimizer/test_pure.py` 新增 8 项单测——4 项 chain-skip（plan-style ready 不跳过 / skipped 仍跳过 / ready 无 steps 仍判空 / 运行时与 constants 双副本行为一致）+ 4 项 cost tracking（单次累计 / 多次累计 / cache+reasoning token / 缺 usage 为 no-op）；`TestToolContracts` + `TestCostTracking` 14 项通过，全量 261 项通过。
...
| P2.7 | `strategy_library.py`、`optimizer/pure/context_snapshot.py`、`optimizer/nodes/subgraphs/phase_select_strategy.py` | `phys_opt` 类策略（PhysOpt / PhysOptAggressive / MUXFTreeReorder，链式 `vivado_phys_opt_design`/`physopt_and_route`）在 WNS<-0.5ns 时被 Vivado 明确告警无效（Physopt 32-745），却仍可选可执行；运行中 phys_opt 在 WNS=-0.542 执行两次零增益、~26s 浪费 | 新增 `PHYSOPT_CLASS_STRATEGIES` + `PHYSOPT_INEFFECTIVE_WNS_THRESHOLD=-0.5`；`context_snapshot` 在 WNS<阈值时将三类策略标 `[BLOCKED]`（catalog 展示），`phase_select_strategy` 新增 `_get_wns_ineffective_strategies` 并入阻断集（执行层拒绝），引导 LLM 改用 route-directive 探索。**2026-07-12 软化（Option C，run-20260711_230953）**：阈值放宽至 -0.75，判定逻辑抽到纯函数 `physopt_ineffective_strategies(latest_wns, initial_wns)`（SELECT 守卫与 Dashboard `[BLOCKED]` 共用 SSOT，避免分歧）--其下仅在 `latest_wns <= initial_wns`（未改善）时阻断，改善中放行 phys_opt；试过失败的 `failed_strategies` TTL 仍兜底再阻断 |
...
**验证**：`tests/test_p2_fixes.py` 新增 7 项单测--P2.7（常量 / catalog 阻断 / WNS 守卫三态）+ P2.8（is_placed 解析 / 失败保守返回）+ P2.10（同文本去重 / 异文本均保留），7 项通过。`optimizer/test_pure.py` + `tests/test_context_engineering_fixes.py` 251 项通过。
