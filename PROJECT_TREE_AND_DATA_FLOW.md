...
| 策略 catalog 排除 | 按原因分级四态（P2 ②D）：`tool_error`/`data_quality_error` 带 `detail` + 剩余重试次数标 `[RETRY: ...]`（可重试）；`regression`（TTL=2）+ 结构性 `strategy_ineffective`（TTL=1）标 `[BLOCKED]`（真阻断，不可选）；`no_improvement`（TTL=3）/`strategy_not_applicable`（TTL=5）标 `[PRIOR FAIL]`（软，可选带上下文）；升级组合（param_signature!=""）标 `[COMBO COOLED]`。`CUSTOM` 策略无主工具，EXECUTE 不收窄工具集 |
